...
/** A group is active while any visible member runs itself or a direct-catalog child. */
function groupHasActivity(
  members: readonly SessionSummary[],
  list: SessionListState,
  statuses: SessionStatuses,
): boolean {
  return members.some(member => member.running || runningChildCount(list, member.id, statuses) > 0)
}
...
export function deriveGroups(
  ...
  for (const g of groupByWorkspace(list, workspaces, archived, rowState.archivedFilter, view.ungroupedOrder)) {
    ...
    groups.push({
      ...
      hasActivity: groupHasActivity(g.sessions, list, statuses),
      sessions: expanded
