...
describe('deriveGroups', () => {
  ...
  it('marks a group active while any member runs, even collapsed, and idle otherwise', () => {
    ...
    const active = deriveGroups(
      list(running, idle), [workspace('first', ['running', 'idle'])], noRows, noAttention, view(),
    )
    ...
    const delegated = deriveGroups(
      { ...withMain(list(descendant, child), child.id), projectionsBySession: { [descendant.id]: catalog('child') } },
      [workspace('first', ['owner', 'child'])], noRows, noAttention, view(),
    )
    expect(delegated[0]!.hasActivity).toBe(true)

    const quiet = deriveGroups(list(idle), [workspace('first', ['idle'])], noRows, noAttention, view())
    expect(quiet[0]!.hasActivity).toBe(false)
    const empty = deriveGroups(list(), [workspace('empty', [])], noRows, noAttention, view())
