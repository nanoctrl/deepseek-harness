...
describe('deriveGroups', () => {
  ...
  it('marks a group active while any member runs, even collapsed, and idle otherwise', () => {
    ...
    const active = deriveGroups(
      list(running, idle), [workspace('first', ['running', 'idle'])], noArchive, view(),
    )
    ...
    const delegated = deriveGroups(
      { ...list(descendant, child), current: child.id }, [workspace('first', ['owner', 'child'])], noArchive, view(),
    )
    expect(delegated[0]!.hasActivity).toBe(true)

    const quiet = deriveGroups(list(idle), [workspace('first', ['idle'])], noArchive, view())
    expect(quiet[0]!.hasActivity).toBe(false)
    const empty = deriveGroups(list(), [workspace('empty', [])], noArchive, view())
