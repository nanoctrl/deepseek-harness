...
describe('registry session removal', () => {
  it('removes a session from its workspace account and the archive set durably', async () => {
    ...
    const removed = await result.registry.removeSession(SessionId('gone'))
    expect(removed).toBe(true)
    // Entities are rebuilt after the removal; read the fresh snapshot.
    expect(result.registry.list()[0]!.sessionIds).toEqual(['kept'])
    expect(result.registry.archivedSessionIds).toEqual([])
    expect(storedState(result.pool).archivedSessionIds).toEqual([])
    const stored = result.pool.media.get('workspace')!.tables.get('workspaces')!.get(workspace.id) as WorkspaceRecord
    expect(stored.sessionIds).toEqual(['kept'])
  })

  it('removes a session accounted only in the archive set and reports unknown ids as a no-op', async () => {
    const dir = await makeDir('remove-archive-only')
    const result = await harness({ sessions: [header('a1', dir, 100)] })
    await result.registry.archiveSession(SessionId('a1'))
    await expect(result.registry.removeSession(SessionId('a1'))).resolves.toBe(true)
    expect(result.registry.archivedSessionIds).toEqual([])

    const unknown = await result.registry.removeSession(SessionId('ghost'))
    expect(unknown).toBe(false)
    expect(storedState(result.pool).archivedSessionIds).toEqual([])
  })

  it('reports false for a session with no registry accounting after its workspace is deleted', async () => {
    const dir = await makeDir('remove-ungrouped')
    const result = await harness({ sessions: [header('u1', dir, 100), header('u2', dir, 200)] })
    const workspace = result.registry.list()[0]!
    await result.registry.delete(workspace.id)
    expect(result.registry.list()).toEqual([])

    const removed = await result.registry.removeSession(SessionId('u1'))
